...
class GuessTheWordGame {
  ...
  async startGame() {
    ...
    const confirmMessages = {
      portuguese:
        "Todos os jogadores estão prontos? Clique em OK para começar.",
      english: "Are all players ready? Click OK to start.",
      spanish:
        "¿Todos los jugadores están listos? Haga clic en Aceptar para comenzar.",
    };
    ...
    const confirmMessage =
      confirmMessages[this.language] || confirmMessages.english;
    ...
    this.playerWords = Array(this.playerCount)
      .fill()
      .map(() => []);
  ...
  async selectRandomWords() {
    try {
      ...
      this.currentWords = await this.fetchWordsFromChatGPT(
        this.language,
        this.difficulty,
        this.wordsPerRound
      );
      ...
      this.currentWords = this.getFallbackWords(
        this.language,
        this.difficulty,
        this.wordsPerRound
      );
  ...
  async fetchWordsFromChatGPT(language, difficulty, count = 5) {
    ...
    if (!this.apiKey) {
      this.apiKey = prompt(
        "Please enter your OpenAI API key (it will be stored in your browser):"
      );
      if (this.apiKey) {
  ...
  difficultyDescription(difficulty, lang) {
    const descriptions = {
      pt: {
  ...
  getWordCountText() {
    const wordCountTexts = {
      portuguese: `Palavra ${this.currentWordIndex + 1} de ${
        this.wordsPerRound
      }`,
  ...
  async endPlayerTurn() {
    ...
    const confirmMessages = {
      ...
      }, está pronto? Clique em OK para começar.`,
      english: `Player ${
        this.currentPlayerIndex + 1
      }, are you ready? Click OK to start.`,
      spanish: `Jugador ${
        this.currentPlayerIndex + 1
      }, ¿está listo? Haga clic en Aceptar para comenzar.`,
    };
    ...
    if (!isReady) {
      ...
      this.currentPlayerIndex =
        (this.currentPlayerIndex - 1 + this.playerCount) % this.playerCount;
  ...
  endGame() {
    ...
    this.finalScoresDiv.innerHTML = this.playerScores.hits
      .map((hits, index) => {
        ...
        const wordListHTML = this.playerWords[index]
          .map((wordItem) => {
